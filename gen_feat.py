...
def extract_feature(path, train, test, type, feat_names):
    ...
    # save label for train/test
    with open("%s/train.true.pkl" %(path), "wb") as f:
        pickle.dump(y_train, f, -1)
    with open("%s/%s.true.pkl" %(path, type), "wb") as f:
        pickle.dump(y_test, f, -1)


    # extract feature for each type
